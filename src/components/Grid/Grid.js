...
const Grid = () => {
        ...
        setArr(prevArr => prevArr.map((row, i) =>
            row.map((el, j) => {
                if (rowIndex === i && columnIndex === j) {
                    ...
                    if (cellSettingType === "weight") {
                        setWeights(prevWeights => [...prevWeights, {x: j, y: i}]);
                        return cellTypes.cell;
                    }
    ...
    const visualizeButtonClickHandler = async () => {
        ...
        switch (chosenAlgorithm) {
            case "bfs":
                await BFS(arr, start, end, onCellClick);
                break;
            case "dfs":
                DFS(arr, start, end, onCellClick);
                break;
            case "dijkstra":
            case "astar":
                await DijkstraAndAStar(arr, start, end, weights, weightCost, onCellClick, chosenAlgorithm);
                break;
            case "gbfs":
                await GBFS(arr, start, end, onCellClick);
                break;
            default:
                break;
        }
    ...
    const genMaze = throttle(() => {
        const maze = generateMaze(m, n);
        setArr(maze[0]);
        setStart(maze[1]);
        setEnd(maze[2]);
    }, 2000);
    const onGenerateMaze = useCallback(genMaze, []);
